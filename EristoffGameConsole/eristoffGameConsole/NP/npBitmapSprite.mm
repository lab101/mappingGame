...
bool npBitmapSprite::checkTouch(ci::Vec2f touch,float scale)
{
    float multiplier = scale -1;
    if(rotation ==0 && z==0 && alpha > 0)
    {
        if((p1t.x - (width/2 *multiplier))  < touch.x
           && (p1t.y - (height/2 *multiplier)) <touch.y
           && (p4t.x + (width/2 *multiplier)) >touch.x
           && (p4t.y + (height/2 *multiplier)) > touch.y )
        {
            return true;
        }
    }
